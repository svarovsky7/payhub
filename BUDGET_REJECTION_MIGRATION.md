# Миграция для обработки отклонения счетов в бюджетировании

## Описание
Данная миграция добавляет логику для автоматического уменьшения суммы "Потрачено" в бюджетировании при отклонении счетов.

## Как выполнить

1. Перейдите в Supabase Studio: https://rydbtsreeggvoblspqxw.supabase.co
...
1. **Обновляет функцию `update_project_budget_on_approval`**:
   - Сохраняет существующую логику увеличения `spent_amount` при одобрении счета
   - Добавляет новую логику уменьшения `spent_amount` при отклонении счета
   - Создает запись в истории бюджета при отклонении

2. **Обновляет view `budget_summary`**:
   - Корректно рассчитывает `remaining_amount` как разность между выделенным и потраченным
   - Упрощает группировку для лучшей производительности

## Логика работы

- При отклонении счета (`NEW.status = 'rejected'`) проверяется, был ли он ранее в стадии `supply_review`, `payment_processing` или `paid`
- Если да, то сумма счета вычитается из `spent_amount` проекта
- Создается запись в `budget_history` с отрицательной суммой и описанием "refunded to budget"
- Используется `GREATEST(spent_amount - NEW.total_amount, 0)` чтобы сумма не стала отрицательной

## После выполнения

Проверьте, что миграция выполнилась успешно, отклонив любой счет и убедившись, что сумма "Потрачено" в бюджетировании уменьшилась на соответствующую сумму.